...
# nd - number of tetrahedra in lower component (missing one)
...
filename = "..\\mesh\\eegapd_%d_%d_%d.1"%(nu,nd0,nd1)
...
def D1index(i):
    if (i > nd1): return -1
    else: return i + Uindex(nu+1)
 
def D0index(i):
    if (i > nd0): return -1
    else: return i + D1index(nd1)
 
D0, D1 = [],[]
D0N, D1N = [],[]
 
if (nd1>0):
    D1.append((v0,d(1),v3,v2))
    for i in range(2,nd1+1):
        D1.append((d(i-1),d(i),v3,v2))
 
    D1N.append((D1index(2),Xindex,-1,-1))
    for i in range(2,nd1+1):
        D1N.append((D1index(i+1),D1index(i-1),-1,-1))
 
if (nd0>0):
    D0.append((v1,d(nd),v2,v3))
    for i in range(nd,nd+1-nd0,-1):
        D0.append((d(i),d(i-1),v2,v3))
 
    D0N.append((D0index(2),Xindex,-1,-1))
    for i in range(2,nd0+1):
        D0N.append((D0index(i+1),D0index(i-1),-1,-1))
...
Xneigh = (D0index(1),D1index(1),Uindex(nu+1),Uindex(1))
